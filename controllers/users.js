...
module.exports = {
  get: catchAsync(async (req, res, next) => {
    try {
      ...
      if (!response.length)
        return next(createHttpError(404, "No usuarios en al base de datos"));
      endpointResponse({
        res,
  ...
  post: catchAsync(async (req, res, next) => {
    ...
    try {
      ...
      if (emailExists) {
        const httpError = createHttpError(403, "Este usuario ya existe");
  ...
  put: catchAsync(async (req, res, next) => {
    ...
    try {
      ...
      if (!user) {
        const httpError = createHttpError(404, "Este usuario no exsite");
        return next(httpError);
      }
  ...
  uploadImage: catchAsync(async (req, res, next) => {
    ...
    try {
      ...
      endpointResponse({ res, message: "Avatar creado con exito" });
    } catch (error) {
      const httpError = createHttpError(
        error.statusCode,
